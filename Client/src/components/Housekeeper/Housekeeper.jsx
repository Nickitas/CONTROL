...
import { Key, Table, Landmark, DoorOpen, FaceMask } from '../../svg.module'
...
const Housekeeper = ({lvl}) => {
    ...
    if(way=='RoomsList'&[0].includes(lvl)) {
        return <RoomsList setWay={setWay} />
    }
            ...
            <section className={classes.Housekeeper}>
                <div className='container'>
                    ...
                    <div className={classes.buttons_list}>
                        ...
                        {[0].includes(lvl)&&<ExtraBtn onClick={() => {setWay('RoomsList')}}>
                            <DoorOpen/>
                            <span>Список комнат</span>
                        </ExtraBtn>}
                        {[0].includes(lvl)&&<ExtraBtn onClick={() => {setWay('DisturberList')}}>
                            <FaceMask/>
                            <span>Список наркшителей</span>
                        </ExtraBtn>}
                    </div>
                </div>
            </section>
