...
import {getRoomsList, getElJournal, getSubunits, getUserKeyList, updateKeyStatus, updateSignalStatus, getDisturbers, addRoom, getRoomEdit, editRoom, removeRoom, getKitKeys } from "./logic/housekeeper.js";
...
const isAuthMethod = async(res, type, body) => {
    ...
    }else if (type == "getRoomEdit") {
        return getRoomEdit(body, (response) => {
            res.send(response)
        })
    }else if (type == "editRoom") {
        return editRoom(body, (response) => {
            res.send(response)
        })
    }else if (type == "removeRoom") {
        return removeRoom(body, (response) => {
            res.send(response)
        })
    }else if (type == "getKitKeys") {
        return getKitKeys(body, (response) => {
            res.send(response)
        })
    }
}
